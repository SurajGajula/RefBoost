...
exports.handler = async (event) => {
  ...
  const { username, campaign } = event.body ? JSON.parse(event.body) : event;
  
  if (!username || !campaign) {
    return { 
      statusCode: 400, 
      headers,
      body: JSON.stringify({ message: 'Missing username or campaign' })
    };
  }
  ...
  if (!campaigns[username]) campaigns[username] = [];
  campaigns[username].push(campaign);
  ...
  try {
    ...
    return { 
      statusCode: 200, 
      headers,
      body: JSON.stringify({ message: 'Campaign saved successfully' })
    };
  } catch (err) {
    return { 
      statusCode: 500, 
      headers,
      body: JSON.stringify({ message: 'Error saving campaign: ' + err.message })
    };
  }
}; 